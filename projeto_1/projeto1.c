...
void voo() /* Voo */
{
    ...
    if(error.found == 0)
    {
        if((v.duracao.hour + v.hora_partida.hour < 24 
        && v.duracao.min + v.hora_partida.min < 60)
        || (v.duracao.hour + v.hora_partida.hour < 23))
        {
            v.hora_chegada = hour_add(v.duracao, v.hora_partida);
...
int less_voo_partida(Voo v1, Voo v2) /* Se a data de partida v1 < data de partida v2 returns 0 else return -1*/
{
...
int less_voo_chegada(Voo v1, Voo v2) /* Se a data de chegada v1 < data de chegada v2 returns 0 else return -1*/
{
    if(v1.data_chegada.ano < v2.data_chegada.ano) return 0;

    if(v1.data_chegada.ano == v2.data_chegada.ano
    && v1.data_chegada.mes < v2.data_chegada.mes) return 0;

    if(v1.data_chegada.ano == v2.data_chegada.ano
    && v1.data_chegada.mes == v2.data_chegada.mes
    && v1.data_chegada.dia < v2.data_chegada.dia) return 0;

    if(v1.data_chegada.ano == v2.data_chegada.ano
    && v1.data_chegada.mes == v2.data_chegada.mes
    && v1.data_chegada.dia == v2.data_chegada.dia
    && v1.hora_chegada.hour < v2.hora_chegada.hour) return 0;

    if(v1.data_chegada.ano == v2.data_chegada.ano
    && v1.data_chegada.mes == v2.data_chegada.mes
    && v1.data_chegada.dia == v2.data_chegada.dia
    && v1.hora_chegada.hour == v2.hora_chegada.hour
    && v1.hora_chegada.min < v2.hora_chegada.min) return 0;
...
void partida(int type) /* Lista os Voos com partida do Aeroporto */
{
    ...
    if(type == 0)
    {
        ...
        for (i = 0; i < count; i++)
        {
            ...
            for (j = count; j > i; j--)
            {
                if(less_voo_partida(voos_sorted[j], voos_sorted[j-1]) == 0 && 
    ...
    if(type == 1)
    {
        ...
        for (i = 0; i < count; i++)
        {
            ...
            for (j = count; j > i; j--)
            {
                if(less_voo_chegada(voos_sorted[j], voos_sorted[j-1]) == 0 && 
                strcmp(voos_sorted[j].codigo, "") != 0 
                && strcmp(voos_sorted[j-1].codigo, "") != 0)
                {
                    temp = voos_sorted[j];
    ...
    if(type == 1)
    {
        ...
        {   
            if (strcmp(voos_sorted[i].id_chegada, id) == 0)
            {
                    printf("%s %s %02d-%02d-%04d %02d:%02d\n", 
                    voos_sorted[i].codigo, 
                    voos_sorted[i].id_partida, 
                    voos_sorted[i].data_chegada.dia, 
                    voos_sorted[i].data_chegada.mes, 
                    voos_sorted[i].data_chegada.ano,
                    voos_sorted[i].hora_chegada.hour, 
                    voos_sorted[i].hora_chegada.min);
